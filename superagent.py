...
from conf import FLAGS, LLM_BASE_URL
...
async def register_reme(agent: ReActAgent):
    async def reme_pre_reasoning(agent: ReActAgent,kwargs):
        ...
        msg_to_keep,compressed_summary=await reme.pre_reasoning_hook(
            ...
            max_input_length=100*1000,
            compact_ratio=0.6, # 0.6倍，超过60K就压缩
...
async def agent_runner(sess: Session):
    while True:
        ...
        if status==SessionStatus.INACTIVE:
            await SESS_MGR.delete_session(sess.session_id)
            await sess.release()
            break
        ...
        try:
            ...
            await session.load_session_state(session_id=session_id,memory=agent.memory)
