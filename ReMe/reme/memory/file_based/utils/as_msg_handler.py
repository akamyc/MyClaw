...
class AsMsgHandler:
    ...
    @staticmethod
    def _estimate_image_tokens(base64_data: str) -> int:
        """Estimate token count for a base64 image based on pixel dimensions."""
        try:
            from PIL import Image
            image_bytes = base64.b64decode(base64_data)
            image = Image.open(io.BytesIO(image_bytes))
            width, height = image.size
            return max(int((width * height) / (32 * 32)), 100)
        except Exception:
            return 1500
    ...
    async def stat_message(self, message: Msg) -> AsMsgStat:
        ...
        for block in message.content:
            ...
            if block_type == "text":
                ...
                if source.get("type") == "base64":
                    data = source.get("data", "")
                    token_count = self._estimate_image_tokens(data) if data else 10
                else:
